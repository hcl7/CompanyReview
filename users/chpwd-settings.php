...
if (isset($_SESSION['uid'])){
  $uid = $_SESSION['uid'];
  if($_POST['userPasswordOld'] != "" && $_POST['userPasswordNew'] != "" && $_POST['userPasswordNewConfirmation'] != ""){
    $old = $_POST['userPasswordOld'];
    $pass = $_POST['userPasswordNew'];
    $repass = $_POST['userPasswordNewConfirmation'];
    if ($bind->checkUserPass($old, $uid)){
      if (strcmp($pass, $repass) == 0){
        $bind->changeUserPassword($pass, $uid);

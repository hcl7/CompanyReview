...
class pafap_csingle_files extends pafap_file_upload {
	...
	function upload_multi_files () {
		...
		if ($this->count_files()) {
			foreach ($this->names_array as $key => $value) {
				if ($value != "") {
					$this->the_file = $value;
					$new_name = $this->set_file_name('', $_SESSION['companyid']);
...
if(isset($_SESSION['companyid']))
{
  ...
  $max_size = 1024*6144;;
  $single_upload = new pafap_csingle_files;
  $single_upload->upload_dir = "../companies/ImagesPool/";
  ...
  if(isset($_FILES['upload']))
  {
    ...
    foreach($single_upload->renamed as $key=>$ren)
    {
      if ($single_upload->process_image($ren)){
        $sql = "UPDATE vlereso_companies SET image = 'companies/ImagesPool/{$ren}' WHERE companyid = '{$cuid}'";
        $multi_mysql->ExecuteSQL_Records_Affected($sql);
        $_SESSION['cimage'] = "companies/ImagesPool/{$ren}";
      }
    }
  }
